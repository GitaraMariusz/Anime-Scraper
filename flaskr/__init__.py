...
def scrape():
    try:
        ...
        if response_data.get('status') == 'success':
            ...
            app.config['ANIME_LIST'] = scraped_anime_list  # Set ANIME_LIST to the scraped list initially
...
def result():
    try:
        ...
        logging.debug(f"Anime list length: {len(anime_list)}")
        logging.debug(f"Page: {page}, Sort by: {sort_by}, Sort order: {sort_order}")
        ...
        logging.debug(f"Paginated anime list: {paginated_anime_list}")
...
def filter():
    try:
        ...
        anime_list = app.config.get('SCRAPED_ANIME_LIST', [])
        filtered_list = [anime for anime in anime_list if any(genre in anime['genres'] for genre in selected_genres)]

        logging.debug(f"Filtered anime list: {filtered_list}")

        app.config['ANIME_LIST'] = filtered_list
        return redirect(url_for('result', page=1))
    except Exception as e:
...
def search():
    try:
        ...
        anime_list = app.config.get('SCRAPED_ANIME_LIST', [])
        search_results = [anime for anime in anime_list if search_query in anime['title'].lower()]

        logging.debug(f"Searched anime list: {search_results}")

        app.config['ANIME_LIST'] = search_results
        return redirect(url_for('result', page=1))
    except Exception as e:
